...
class Matcher {
	 ...
	 * @return array
	 */
	public static function tryRouteForUrl(\Mmi\Controller\Router\Config\Route $route, $url) {
		$params = [];
		$matches = [];
		$matched = false;
		...
		if (self::_isPatternRegular($route->pattern) && preg_match($route->pattern, $url, $matches)) {
			...
			foreach ($route->replace as $key => $value) {
				self::$_tmpKey = $key;
				$params[$key] = preg_replace_callback('/\$([0-9]+)/', ['\Mmi\Controller\Router\Matcher', '_routeMatch'], $value);
	 ...
	 * @return array
	 */
	public static function tryRouteForParams(\Mmi\Controller\Router\Config\Route $route, array $params) {
		$matches = [];
		$matched = [];
		$applied = true;
		$url = '';
	...
	protected static function _routeMatch($matches) {
		...
		throw new\Exception('Router failed due to invalid route definition - no default param for key: ' . self::$_tmpKey);
	}
